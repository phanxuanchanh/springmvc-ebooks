							...
							<tbody>
								<c:forEach var="item" items="${ bookAuthors }">
									<tr role="row">
										...
										<td>${ item.name }</td>
										<td><a
											href="<c:url value="/quan-tri/chinh-sua-tac-gia/${ item.Id }"></c:url>">Chỉnh
												sửa</a> | <a
											href="<c:url value="/quan-tri/chi-tiet-tac-gia/${ item.Id }"></c:url>">Xem
												chi tiết</a> | <a
											href="<c:url value="/quan-tri/xoa-tac-gia/${ item.Id }"></c:url>">Xóa</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
